...
Shared truecasing engine used by the separate Stanza and FLAN-T5 runners. It
combines a deterministic sentence-case baseline, a file-specific canonical-name
lexicon, and token-level evidence supplied by a resolver. Every output row has
the same schema as its input row; ``text`` may change case and restore explicitly
configured apostrophes.
"""
...
def write_jsonl(
    ...
    source_rows: Sequence[dict], results: Sequence[dict],
) -> None:
    ...
    changed = sum(a["text"] != b["text"] for a, b in zip(source_rows, results))
