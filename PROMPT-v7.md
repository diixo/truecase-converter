...
2. Файл делится на stateless-блоки по 50 target-записей.
3. Каждый модельный запрос получает `5 context_before + 50 targets + 5 context_after`.
...
If the 5+50+5 window is insufficient, put the affected target index in
...
- Решение применяется ко всем точным употреблениям `source` только внутри текущих 50 target-записей.
